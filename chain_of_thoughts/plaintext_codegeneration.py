...
async def run_chain(template_str: str, input_vars: dict, escape_keys: list = None) -> str:
    ...
    Returns:
        str: The content generated by the chain.
    """
    if escape_keys:
...
async def generate_test_code(refined_scenario: str) -> str:
    """
    ...
    result = await run_chain(template_str, {"refined_scenario": refined_scenario},
                             escape_keys=["refined_scenario"])
    return result


async def validate_and_improve_code(generated_code: str) -> str:
    """
    ...
    improved_code = await run_chain(template_str, {"generated_code": generated_code},
                                    escape_keys=["generated_code"])
...
async def main():
    try:
        ...
        logger.info("<<<Obtained refined scenario from preprocessing module.\n")

        # Step 3: Generate the test code based on the refined scenario.
        test_code = await generate_test_code(refined_scenario)
        logger.info(f"<<<Generated test code successfully.\n{test_code}")

        # Step 4: Validate and improve the generated code.
        improved_test_code = await validate_and_improve_code(test_code)
